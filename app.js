                                    ...
                                    try {
                                                        ...
                                                        if (colNombre !== -1) {
                                                                                resultado = filas.slice(filaInicio)
                                                                                    ...
                                                                                    .map(f => {
                                                                                                            const direccionBase = String(f[colDireccion]).trim();
                                                                                                            const ciudadTexto = colCiudad !== -1 && f[colCiudad] ? String(f[colCiudad]).trim() : '';
                                                                                                            // Algunos Excel ya traen la ciudad/estado/zip pegados dentro de la
                                                                                                            // misma columna de Direccion (ej: "Calle, Graham, NC 12345") Y ADEMAS
                                                                                                            // tienen una columna aparte con el estado o la ciudad repetidos. Si
                                                                                                            // pegaramos esa columna de nuevo, la direccion quedaba duplicada
                                                                                                            // ("...NC 12345, NC"), lo que confundia al navegador GPS. Por eso
                                                                                                            // solo la pegamos si todavia no aparece dentro de la Direccion.
                                                                                                            const escapado = ciudadTexto.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                                                                                                            const yaIncluida = ciudadTexto && new RegExp(`(^|[,\\s])${escapado}([,\\s]|$)`, 'i').test(direccionBase);
                                                                                                            const direccionFinal = ciudadTexto && !yaIncluida ? `${direccionBase}, ${ciudadTexto}` : direccionBase;
                                                                                                            return [
...
async function construirRuta(manager) {
  ...
  if (pendientes.length > 0 && (!manager.puntoInicio || manager.puntoInicio.lat == null || manager.puntoInicio.lng == null)) {
    ...
    if (pendientes.length > 0) {
          // El punto de inicio fijo (definirPuntoInicio()) SOLO se usa para arrancar el
          // dia, cuando el manager todavia no ha gestionado a nadie. En cuanto ya tiene
          // al menos una visita hecha hoy, seguimos calculando el resto desde la ULTIMA
          // parada real (no desde el punto de inicio otra vez) — asi, si la app se
          // recarga a mitad del dia (se bloqueo el celular, se fue la señal, etc.), la
          // ruta continua desde donde el manager esta parado de verdad en vez de saltar
          // de vuelta al punto de inicio y desordenar todo lo que le falta.
          const ultimoCompletado = completados.length > 0 ? completados[completados.length - 1] : null;
          const punto0 = (ultimoCompletado && ultimoCompletado.lat != null && ultimoCompletado.lng != null)
                ? { lat: ultimoCompletado.lat, lng: ultimoCompletado.lng }
                : { lat: manager.puntoInicio.lat, lng: manager.puntoInicio.lng };
          let punto = punto0;
  ...
  rutaOrdenada = [...completados, ...pendientesOrdenados, ...pendientesSinCoords];
    ...
    // Lo aplicamos DE UNA VEZ sobre los datos que ya tenemos cargados en este celular
    // (estado.clientes), para que si el manager descarga su Excel inmediatamente
    // despues de entrar a su ruta, la descarga ya salga en este mismo orden sin tener
    // que esperar. Antes esto solo se guardaba de fondo en el servidor y, si el manager
    // descargaba muy rapido, la descarga podia salir desordenada porque el guardado
    // todavia no habia terminado.
    estado.clientes.forEach(c => {
        if (ordenPorId[c.id] !== undefined) c.ordenRuta = ordenPorId[c.id];
    });

    // Ademas lo guardamos de fondo en el servidor (para que quede disponible desde
    // otro celular o desde el Panel Admin). No bloqueamos la pantalla esperando a que
    // termine: el manager ya puede seguir trabajando mientras esto se guarda.
