...
function partirDireccionUS(direccion) { if (!direccion) return { calle: '', ciudad: '', estado: '', zip: '' }; const partes = direccion.split(',').map(p => p.trim()).filter(Boolean); if (partes.length >= 3) { const calle = partes[0]; const ciudad = partes[1]; const resto = partes[2]; const match = resto.match(/^([A-Za-z]{2})\s+(\d{5}(-\d{4})?)/); if (match) { return { calle, ciudad, estado: match[1], zip: match[2] }; } return { calle, ciudad, estado: resto, zip: '' }; } if (partes.length === 2) { return { calle: partes[0], ciudad: partes[1], estado: '', zip: '' }; } return { calle: direccion, ciudad: '', estado: '', zip: '' }; } function abrirCalendarioYAgendar() { const manager = estado.managers.find(m => m.id === managerActivoId); const c = rutaOrdenada[indiceClienteActual]; if (!manager || !manager.calendarioUrl) { alert('Este manager todavia no tiene un link de calendario configurado. Pidele al administrador que lo agregue en el panel de administrador con el boton Calendario.'); return; } const dir = partirDireccionUS(c.direccion || ''); const params = new URLSearchParams({ full_name: c.nombre || '', address: dir.calle, city: dir.ciudad, state: dir.estado, postal_code: dir.zip }); window.open(manager.calendarioUrl + '?' + params.toString(), '_blank'); marcarEstatus('cita'); } function mostrarFormCita() {
